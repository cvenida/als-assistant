    ...
    <v-card class="rounded-2xl p-4">
      <div class="flex items-start gap-4 p-2">
        ...
        <div>
          ...
          <p class="text-xs text-neutral-700 font-medium mt-1 leading-relaxed">
            <template v-if="message">
              {{ message }}
            </template>
            <template v-else-if="itemName">
              Are you sure you want to delete <strong class="text-neutral-950 font-bold">"{{ itemName }}"</strong>? This action cannot be undone.
            </template>
            <template v-else>
      ...
      <div class="flex justify-end gap-2 mt-6">
        <v-btn
          variant="outlined"
          rounded="lg"
          class="capitalize font-semibold border-neutral-300 text-neutral-800"
          :disabled="isLoading"
          @click="handleClose"
        >
          Cancel
